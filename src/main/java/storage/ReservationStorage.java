...
public class ReservationStorage {
    ...
    /**
     * Retourne toutes les réservations actives (EN_ATTENTE ou CONFIRMEE) liées à
     * une agence.
     * Utilisé par le gestionnaire pour consulter le planning de son agence.
     */
    public static List<Reservation> getReservationsByAgency(String agence) {
        List<Reservation> result = new ArrayList<>();
        if (agence == null)
            return result;
        for (Reservation r : reservations.values()) {
            // Filtre sur l'agence de départ ou de retour
            if (agence.equalsIgnoreCase(r.getAgenceDepart())
                    || agence.equalsIgnoreCase(r.getAgenceRetour())) {
                result.add(r);
            }
        }
        return result;
    ...
    /**
     * Vérifie s'il existe un conflit de réservation pour un véhicule sur une
     * période donnée.
     */
    public static boolean hasConflict(Long vehicleId, LocalDate dateDebut, LocalDate dateFin) {
        return isVehicleReservedForPeriod(vehicleId, dateDebut, dateFin);
    }
}